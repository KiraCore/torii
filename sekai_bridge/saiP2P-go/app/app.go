...
func Run() {
	...
	go func() {
		if err := http.ListenAndServe(":"+config.P2P.Http.Port, nil); err != nil {
	...
	go func() {
		for {
			...
			core.Logger.Info("MESSAGE", zap.String("FROM", msg.From), zap.Strings("TO", msg.To), zap.Int("LEN", len(msg.Data)))
		}
	}()
